...
namespace ToastClient.Modules
{
    class Misce : BaseModule
    {
        ...
        public static void AvatarLog()
        {
            var Players = Wrappers.GeneralWrappers.GetAllPlayers();
            for (int i = 0; i < Players.Length; i++)
            {
                var player = Players[i];
                var playeravatar = Wrappers.GeneralWrappers.GetApiAvatar(player);
                MelonLogger.Msg("author: " + playeravatar.authorName);
                MelonLogger.Msg("author id: " + playeravatar.authorId);
                MelonLogger.Msg("name: " + Wrappers.GeneralWrappers.GetAPIUser(player).displayName);
                MelonLogger.Msg("user'd id: " + Wrappers.GeneralWrappers.GetAPIUser(player).id);
                MelonLogger.Msg("avatur id: " + playeravatar.id);
                MelonLogger.Msg("asset Url: " + playeravatar.assetUrl);
                MelonLogger.Msg("avatur image: " + Wrappers.GeneralWrappers.GetAPIUser(player).currentAvatarImageUrl);
                MelonLogger.Msg("release status: " + playeravatar.releaseStatus);
            }
        }
        ...
        public static void HighlightPlayer(Player player, bool state)
        {
            if (player != null)
            {
                if (state)
                {
                    ...
                    othersH.highlightColor = GetTrustColor(player);
        ...
        public static void PrisonEscapeGodMod()
        {
            try
            {
                GameObject HealthContainer = GameObject.Find("Scripts/Game Effects/Canvas HUD/Health Canvas/Health Panel/Health Slider");
                var slider = HealthContainer.GetComponent<Slider>();
                

            }
            catch {  }
        }
        public static void PrisonEscapePower()
        {
